...
export const SellerShows: React.FC = () => {
  ...
  const filteredShows = useMemo(() => {
    ...
    return state.sellerShows.filter((s) => {
      if (activeTab !== "ALL" && s.status !== activeTab) return false
      return !term || s.title.toLowerCase().includes(term)
    })
  }, [state.sellerShows, activeTab, search])

  const selectedShow = state.sellerShows.find((s) => s.id === selectedId)
  ...
  return (
    <>
      ...
      >
        {/* Real LiveKit broadcast flow — see app/(broadcast)/sell. */}
        <Link
          to="/sell"
          className="flex items-center gap-2 rounded-full bg-[var(--wn-live)] px-5 py-2.5 text-[14px] font-[700] text-white transition-opacity hover:opacity-90"
        >
