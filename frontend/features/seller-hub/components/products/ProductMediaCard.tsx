...
import { ImagePlus, ImageOff, Loader2, X } from 'lucide-react';
...
export const ProductMediaCard: React.FC<ProductMediaCardProps> = ({ images, onChange }) => {
  ...
  /**
   * Тохиргоогүй үед ч талбарын хэлбэрийг хадгална. Хэрэглэгчид env хувьсагчийн
   * нэр хэлэх нь утгагүй тул зөвхөн юу болсныг хэлнэ — тохируулах хүнд
   * `NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME`, `NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET`
   * хоёр хэрэгтэй (`lib/cloudinary.ts`).
   */
  if (!uploadReady) {
    return (
      <Panel title="Зураг">
        <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-[var(--wn-admin-outline)] bg-[var(--wn-admin-row-rule)] px-6 py-12 text-center">
          <ImageOff className="h-7 w-7 text-[var(--wn-admin-muted)]" />
          <p className="text-[14px] font-[700] text-[var(--wn-admin-ink-2)]">
            Зураг байршуулах боломж идэвхгүй байна
          </p>
          <p className="max-w-[380px] text-[13px] font-[500] text-[var(--wn-admin-muted)]">
            Барааг зураггүйгээр ч хадгалж болно. Зураг нэмэхийн тулд админтайгаа
            холбогдож байршуулалтын тохиргоог идэвхжүүлнэ үү.
          </p>
        </div>
      </Panel>
    );
    ...
    <Panel title="Зураг">
      ...
      >
        ...
        <span className="pointer-events-none flex flex-col items-center gap-2">
          ...
          <span className="text-[13px] font-[500] text-[var(--wn-admin-muted)]">{HINT}</span>
        </span>
      </button>
          ...
          {images.map((url, index) => (
            ...
            >
              ...
              <button
                ...
                className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full border border-[var(--wn-admin-card-border)] bg-white text-[var(--wn-admin-muted)] shadow-sm transition-colors hover:text-black"
              >
                <X className="h-3 w-3" />
              </button>
            </div>
      ...
      {error && (
        <p className="mt-3 text-[13px] font-[600] text-[var(--wn-admin-danger)]">{error}</p>
      )}
    </Panel>
  );
};
