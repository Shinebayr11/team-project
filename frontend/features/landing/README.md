...
⚠️ Апп болон backend нь **зоосоор ажилласаар байна** (`server/src/route/
cointransactionRoute.ts`, `hooks/useWallet.ts`-ийн `coin_balance`,
`components/live/auction-bid-panel.tsx`-ийн `*_bid_coins`). Landing-ийн
амлалт бүтээгдэхүүнтэй таарах ёстой бол тэр migration тусдаа хийгдэнэ.
