...
export const useHomeFeed = (query: string, category: string, statusFilter: StatusFilter = null) => {
  ...
  const hottestShow = useMemo(() => {
    ...
    // эхэлсэн эфир 0 үзэгчтэй байдаг тул үзэгчээр эрэмбэлбэл хуучин мөрөнд
    // хожигдоно гэж үзсэн. Тэр эрсдэл хэвээр: цөөн үзэгчтэй шинэ эфир нүүрэнд
    // шууд гарахгүй байж болно. Ижил үзэгчтэй үед л шинэ нь тэргүүлнэ.
    return [...pool].sort(
      (a, b) => (b.live ?? 0) - (a.live ?? 0) || startedMs(b) - startedMs(a)
    )[0];
  }, [shows]);
