...
import { LiveShowDoc, isOnAir, toHomeShow } from "@/lib/liveShows"
...
export function useLiveShows() {
  ...
  useEffect(() => {
    ...
    callApi<{ data: LiveShowDoc[] }>("/api/liveshow")
      .then((res) => {
        // Яг одоо эфирт байгаа дамжуулалтыг л авна — дууссан болон гацсан
        // мөрүүд нүүрэнд хуурамч "Scheduled" ангилал үүсгэдэг (`isOnAir`).
        //
        ...
        if (!cancelled) {
          setShows([...res.data.filter(isOnAir).map(toHomeShow), ...MOCK_SHOWS])
        }
      })
      .catch((err) => {
